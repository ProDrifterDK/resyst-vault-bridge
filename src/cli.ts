...
import { buildBootstrap, type BootstrapNotes } from "./bootstrap.js";
...
interface HarnessIO {
  ...
  /** True when stdin is an interactive terminal (no piped body). */
  stdinIsTTY: boolean;
  /** Cumulative encoded stdout bytes for this invocation. */
  outputBytes?: number;
}
...
const SearchRequestSchema = Type.Object(
  {
    ...
    limit: Type.Optional(
      Type.Integer({ minimum: MIN_SEARCH_LIMIT, maximum: MAX_SEARCH_LIMIT }),
    ),
  },
  { additionalProperties: false },
);
...
const BootstrapRequestSchema = Type.Object(
  {
    notes: Type.Object(
      {
        ...
        moc: Type.Optional(
          Type.Union([BootstrapSnapshotSchema, Type.Null()]),
        ),
      },
      { additionalProperties: false },
    ),
    ...
    budget_tokens: Type.Optional(
      Type.Integer({ minimum: MIN_BUDGET_TOKENS, maximum: MAX_BUDGET_TOKENS }),
    ),
  },
  { additionalProperties: false },
);
...
type CommandPlan =
  | { kind: "doctor"; cleanAbandonedLock: boolean }
  | { kind: "status" }
  | { kind: "search" }
  | { kind: "read" }
  | { kind: "bootstrap" }
  | { kind: "checkpoint"; dryRun: boolean; apply: boolean }
  | { kind: "recover" }
  | { kind: "rollback"; eventId: string }
  | { kind: "help" };
...
async function readStdinBounded(io: HarnessIO): Promise<string> {
  const chunks: Buffer[] = [];
  let total = 0;
...
function validateBoundedValue(
  value: unknown,
  limits: { depth: number; nodes: number; stringLength: number } = {
  ...
  while (stack.length > 0) {
    ...
    nodes += 1;
    if (nodes > limits.nodes) return FIXED_STDERR.json_nodes;
    if (frame.depth > limits.depth) return FIXED_STDERR.json_depth;
    if (typeof frame.value === "string") {
      if (frame.value.length > limits.stringLength) return FIXED_STDERR.json_string;
      continue;
    }
    ...
    for (let index = keys.length - 1; index >= 0; index -= 1) {
      ...
      if (key.length > limits.stringLength) return FIXED_STDERR.json_string;
...
async function readFramedRequests(
  io: HarnessIO,
  options: { requireBody: boolean },
): Promise<
  | { ok: true; lines: unknown[] }
  | { ok: false; exit: number; stderr: string }
> {
  ...
  if (raw.length === 0) {
    if (!options.requireBody) return { ok: true, lines: [] };
    return {
      ok: false,
      exit: EXIT_INVALID,
...
class CliBootstrapError extends Error {
  constructor(readonly code: "invalid_state_root" | "invalid_config") {
    super(code);
...
function resolveStateRoot(): string {
  ...
  if (xdgState !== undefined && xdgState.length > 0) {
    if (!path.isAbsolute(xdgState)) throw new CliBootstrapError("invalid_state_root");
    return path.resolve(xdgState);
  }
  const home = process.env.HOME ?? os.homedir();
  if (!path.isAbsolute(home)) throw new CliBootstrapError("invalid_state_root");
...
function resolveConfigRoot(): string {
  ...
  if (xdgConfig !== undefined && xdgConfig.length > 0) {
    if (!path.isAbsolute(xdgConfig)) throw new CliBootstrapError("invalid_config");
    return path.resolve(xdgConfig);
  }
  const home = process.env.HOME ?? os.homedir();
  if (!path.isAbsolute(home)) throw new CliBootstrapError("invalid_config");
  return path.resolve(home, ".config");
}

// ---------------------------------------------------------------------------
// Service container. The state root is always an absolute, non-empty string.
// ---------------------------------------------------------------------------
...
interface Services {
  ...
  searchCache: ReturnType<typeof nodeSearchCacheStore>;
  rollback: RollbackService;
}

async function buildServices(stateRoot: string): Promise<Services> {
...
function envelope(command: CommandName, fields: Envelope): Envelope {
...
function safeRecord(value: unknown): Envelope {
  ...
  return (visit(value) ?? {}) as Envelope;
}
...
function writeStdout(io: HarnessIO, value: unknown): void {
  ...
  if (bounded !== null) throw new ResponseLimitError();
...
function stderrForOutcome(outcome: string): string {
  switch (outcome) {
...
function stderrFixed(io: HarnessIO, message: string): void {
  io.stderr.write(message + "\n");
}

function configErrorExit(error: ConfigError): number {
  return error.code.startsWith("vault_") || error.code.startsWith("layout_")
    ? EXIT_UNAVAILABLE
    : EXIT_INVALID;
}

function writeConfigFailure(
  io: HarnessIO,
  command: CommandName,
  error: ConfigError,
): number {
...
function planCommandName(plan: CommandPlan): CommandName {
  if (plan.kind === "rollback") return "rollback";
  if (plan.kind === "checkpoint") return "checkpoint";
  return plan.kind as CommandName;
}

function isBodylessCommand(plan: CommandPlan): boolean {
  switch (plan.kind) {
    case "doctor":
    case "status":
    case "recover":
...
async function runDoctor(
  io: HarnessIO,
  plan: Extract<CommandPlan, { kind: "doctor" }>,
): Promise<number> {
...
async function runStatus(io: HarnessIO): Promise<number> {
  let stateRoot: string;
  try {
    stateRoot = resolveStateRoot();
  } catch (error) {
...
async function runSearch(io: HarnessIO, requests: unknown[]): Promise<number> {
  let decoded: Array<StaticDecode<typeof SearchRequestSchema>>;
  try {
    decoded = requests.map((request) =>
      parseWithSchema(SearchRequestSchema, request, "search request"));
  } catch {
...
async function runRead(io: HarnessIO, requests: unknown[]): Promise<number> {
  let decoded: Array<StaticDecode<typeof ReadRequestSchema>>;
  try {
    decoded = requests.map((request) =>
      parseWithSchema(ReadRequestSchema, request, "read request"));
  } catch {
...
async function runBootstrap(
  ...
  let decoded: Array<StaticDecode<typeof BootstrapRequestSchema>>;
  try {
    decoded = requests.map((request) =>
      parseWithSchema(BootstrapRequestSchema, request, "bootstrap request"));
  } catch {
  ...
  for (const parsed of decoded) {
    ...
    const result = buildBootstrap({
      ...
      estimateTokens: (context: string) => Math.ceil(Array.from(context).length / 4),
    });
...
function applyEventId(idempotencyKey: IdempotencyKey): EventId {
...
function applyOutcomeEnvelope(outcome: CheckpointOutcome): Envelope {
  switch (outcome.kind) {
    case "applied":
...
function applyOutcomeExit(outcome: CheckpointOutcome): number {
  switch (outcome.kind) {
    case "applied":
    case "already_applied":
    case "noop":
...
async function runCheckpoint(
  ...
  const record =
    typeof request === "object" && request !== null && !Array.isArray(request)
      ...
      : null;
  const inner =
    record !== null && typeof record.checkpoint === "object" &&
  ...
  if (kind === "noop") {
    let noop: StaticDecode<typeof CheckpointNoopRequestSchema>;
    try {
      noop = parseWithSchema(
        CheckpointNoopRequestSchema,
        request,
        "checkpoint request",
      );
    } catch {
  ...
  let apply: StaticDecode<typeof CheckpointApplyRequestSchema>;
  try {
    apply = parseWithSchema(
      CheckpointApplyRequestSchema,
      request,
      "checkpoint request",
    );
  } catch {
...
async function runRecover(io: HarnessIO): Promise<number> {
  let stateRoot: string;
  try {
    stateRoot = resolveStateRoot();
  } catch (error) {
...
function runHelp(io: HarnessIO): number {
...
export async function runCli(io: HarnessIO): Promise<number> {
  const parsedArgs = parseArgs(io.argv);
  if (!parsedArgs.ok) {
    const exit = parsedArgs.exit;
    const stderrMessage: string = parsedArgs.stderr;
  ...
  if (!framing.ok) {
    ...
    const stderrMessage: string = framing.stderr;
