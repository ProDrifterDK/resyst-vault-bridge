...
describe("Prime extension integration", () => {
  it("points Git installation at the tracked TypeScript extension source", async () => {
    ...
    expect(extensions).toEqual(["./src/extension/index.ts"]);
    await expect(
      readFile(path.join(process.cwd(), "src/extension/index.ts"), "utf8"),
  ...
  it("registers read tools immediately for both root and child runtimes, never checkpoint", () => {
    for (const depth of [0, 3]) {
      ...
      createVaultExtension({ service: service(vi.fn(async () => "context")) })(fixture.api);
      expect(fixture.tools.map((tool) => tool.name).sort()).toEqual(["vault_read", "vault_search"]);
      expect(fixture.tools.some((tool) => tool.name === "vault_checkpoint")).toBe(false);
      expect(context({ id: `session-${String(depth)}`, rlmDepth: depth })).toBeDefined();
    }
  });
  ...
  it("refuses malformed events, hostile contexts, and non-primitive cwd values", async () => {
    ...
    Object.defineProperty(relativeCtx, "cwd", { value: "relative/cwd", configurable: true });
    ...
    const nonStringEvent = { type: "before_agent_start", prompt: 42, systemPrompt: "BASE", systemPromptOptions: {} };
    const nonStringResult = (
      await emit(fixture, "before_agent_start", nonStringEvent, context(baseHeader))
    )[0];
    expect(nonStringResult).toBeUndefined();

    // Oversized system prompt fails closed.
    const oversizedSystem = "x".repeat(2_000_000);
    const oversizedEvent = { type: "before_agent_start", prompt: "prompt", systemPrompt: oversizedSystem, systemPromptOptions: {} };
    const oversizedResult = (
      await emit(fixture, "before_agent_start", oversizedEvent, context(baseHeader))
    )[0];
    expect(oversizedResult).toBeUndefined();

    // Non-object event fails closed.
    const nonObjectResult = (
      await emit(fixture, "before_agent_start", "string-not-an-event", context(baseHeader))
    )[0];
  ...
  it("fails closed for every child-equivalent header despite spoofable metadata", async () => {
    ...
    for (const header of [
      ...
      const result = (await emit(fixture, "before_agent_start", before(), context(header)))[0];
      expect(result).toBeUndefined();
    }
    expect(bootstrap).not.toHaveBeenCalled();
  });
  ...
  it("keeps the agent running when bootstrap is empty or unavailable", async () => {
    for (const bootstrap of [
      vi.fn(async () => ""),
      vi.fn(async () => { throw new Error("/private/vault/path and note content"); }),
    ]) {
      const fixture = harness();
      createVaultExtension({ service: service(bootstrap) })(fixture.api);
      const result = (await emit(
  ...
  it("production service assembles bootstrap and reads only a synthetic vault", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "resyst-prime-production-"));
    try {
      ...
      await expect(production.search({ query: "Atlas" })).resolves.toMatchObject({ version: 1 });
      await expect(production.read({ path: "Proyectos/Atlas.md" })).resolves.toMatchObject({
        version: 1,
