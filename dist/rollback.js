...
function request(value) {
    ...
    const record = value;
    if (Object.keys(record).some((key) => !["target_event_id", "event_id", "idempotency_key"].includes(key)))
        throw new RollbackIntegrityError();
    try {
        return { target_event_id: parseWithSchema(EventIdSchema, record.target_event_id, "target event"), event_id: parseWithSchema(EventIdSchema, record.event_id, "rollback event"), idempotency_key: parseWithSchema(IdempotencyKeySchema, record.idempotency_key, "rollback key") };
    }
    catch {
        throw new RollbackIntegrityError();
    }
}
function encoded(value) { return Buffer.from(value, "utf8").toString("base64url"); }
function parseBackup(value, event, target) {
    if (typeof value !== "object" || value === null || Array.isArray(value))
        throw new RollbackIntegrityError();
    const record = value;
    const keys = ["version", "event_id", "path", "existed", "mode", "before_hash", "backup_file"];
    if (Object.keys(record).some((key) => !keys.includes(key)) || keys.some((key) => !(key in record)) || record.version !== 1 || record.event_id !== event || record.path !== target || typeof record.existed !== "boolean" || !Number.isInteger(record.mode) || record.mode < 0 || record.mode > 0o7777 || (record.backup_file !== null && typeof record.backup_file !== "string"))
        throw new RollbackIntegrityError();
    let before;
    try {
        before = record.before_hash === null ? null : parseWithSchema(HashHexSchema, record.before_hash, "backup hash");
    }
    catch {
        throw new RollbackIntegrityError();
    }
    return { version: 1, event_id: event, path: target, existed: record.existed, mode: record.mode, before_hash: before, backup_file: record.backup_file };
}
export class RollbackIntegrityError extends Error {
    cause;
    constructor(cause = "invalid_state") {
...
export class RollbackService {
    ...
    constructor(options) {
        ...
        if (options.afterTarget !== undefined)
            this.afterTarget = options.afterTarget;
        if (options.beforeReceipt !== undefined)
            this.beforeReceipt = options.beforeReceipt;
        void options.vaultRoot;
        void options.identity;
    }
    async snapshot(absolute) {
        let linkStat;
        try {
            ...
            if (isMissing(error))
                return { exists: false, hash: null, dev: null, ino: null };
            throw new RollbackIntegrityError("invalid_state");
        }
        ...
        try {
            ...
            if (!opened.isFile() || opened.dev !== linkStat.dev || opened.ino !== linkStat.ino || opened.size > BigInt(MAX_BACKUP_BYTES))
                throw new RollbackIntegrityError("invalid_state");
    ...
    async readBackupFile(file) {
        ...
        try {
            ...
            if (!state.isFile() || state.size > BigInt(MAX_BACKUP_BYTES))
                throw new RollbackIntegrityError("invalid_state");
    ...
    async backup(event, target) {
        ...
        if (record.existed !== (record.before_hash !== null) || record.backup_file !== (record.existed ? `${key}.before` : null))
            throw new RollbackIntegrityError("invalid_state");
        let content = null;
        if (record.backup_file !== null) {
            content = await this.readBackupFile(path.join(directory, record.backup_file));
            if (hash(content) !== record.before_hash)
                throw new RollbackIntegrityError("invalid_state");
        }
        return { record, content };
    }
    async write(absolute, content, mode, expected) {
        ...
        try {
            ...
            if (current.exists !== expected.exists || current.hash !== expected.hash || current.dev !== expected.dev || current.ino !== expected.ino)
                throw new RollbackIntegrityError("invalid_state");
    ...
    async rollback(value) {
        ...
        if (existing?.receipt.outcome === "rolled_back")
            return { kind: "already_rolled_back", event_id: existing.event_id, receipt: existing.receipt };
        let held;
        try {
        ...
        try {
            ...
            if (lockedReceipt !== null) {
                if (lockedReceipt.receipt.outcome === "rolled_back")
                    return { kind: "already_rolled_back", event_id: lockedReceipt.event_id, receipt: lockedReceipt.receipt };
                return { kind: "rejected", reason: "invalid_state" };
            }
            let applied;
            try {
                applied = await this.journal.readReceipt(input.target_event_id);
            }
            catch {
                return { kind: "rejected", reason: "not_applied" };
            }
            if (applied.outcome !== "applied")
                return { kind: "rejected", reason: "not_applied" };
            const expectedTargets = [...applied.targets]
            ...
            if (priorEvent !== undefined && (priorEvent.kind !== "rollback" ||
            ...
            for (const item of resolved) {
                ...
                if (recovery) {
                    ...
                    if (current !== item.target.after_hash && current !== item.target.before_hash)
                        mismatches.push(item.target.path);
                }
                else {
                    if (current !== item.target.after_hash)
                        mismatches.push(item.target.path);
                }
            }
            if (mismatches.length > 0)
                return { kind: "rejected", reason: "precondition_mismatch", mismatch_paths: mismatches.sort() };
            ...
            for (let index = 0; index < sortedItems.length; index += 1) {
                ...
                if (recovery && current.hash === item.target.before_hash)
                    continue;
                if (current.hash !== item.target.after_hash)
                    throw new RollbackIntegrityError("invalid_state");
                if (item.backup.content === null)
                    await this.unlinkCreated(item.path.absolute, current);
                else
                    await this.write(item.path.absolute, item.backup.content, item.backup.record.mode, current);
                if (this.afterTarget !== undefined)
                    await this.afterTarget(item.target.path, index);
            }
            ...
            // the receipt.
            if (this.beforeReceipt !== undefined)
            ...
            if (error instanceof RollbackIntegrityError) {
                if (error.cause === "missing_backup")
                    return { kind: "rejected", reason: "missing_backup" };
                return { kind: "rejected", reason: "invalid_state" };
            }
            throw error;
        }
    ...
    async unlinkCreated(absolute, expected) {
        const parent = path.dirname(absolute);
        const current = await this.snapshot(absolute);
        if (!current.exists || current.hash !== expected.hash || current.dev !== expected.dev || current.ino !== expected.ino)
            throw new RollbackIntegrityError("invalid_state");
        try {
