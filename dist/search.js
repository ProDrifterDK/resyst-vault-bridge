...
import { chmod, mkdir, open, opendir, realpath, rename, stat, lstat, unlink, } from "node:fs/promises";
...
async function readDirectoryBounded(filePath) {
    const handle = await opendir(filePath);
    const entries = [];
    try {
        for await (const entry of handle) {
            if (entries.length >= 256)
                throw new Error("directory entry budget exceeded");
            entries.push(entry);
        }
        return entries;
    }
...
export function nodeSearchCacheStore(options = {}) {
...
function renderQuotedSnippet(pathValue, heading, modifiedAt, source) {
    const quoted = source
        .split(/(\r\n|\r|\n|\u2028|\u2029)/u)
...
function quoteSnippet(pathValue, heading, modifiedAt, source, maxChars = MAX_SNIPPET_CHARS) {
    const full = renderQuotedSnippet(pathValue, heading, modifiedAt, source);
    if (full.length <= maxChars)
        return { snippet: full, truncated: false };
...
function matchClass(value, query) {
...
function scoreNote(note, query) {
    ...
    for (const field of fields) {
        const classification = Math.max(0, ...field.values.map((value) => matchClass(normalize(value), query)));
        if (classification === 0)
            continue;
...
function parseCachedIndex(value) {
    ...
    try {
        ...
        if (candidate.version !== 1 ||
            ...
            return null;
        const vaultRecord = vault;
        if (Object.keys(candidate).length !== 3 || Object.keys(vaultRecord).length !== 2 ||
...
async function verifyTrustedRoot(config, fs) {
...
async function listVaultNotes(options, fs, vaultPaths) {
    const files = [];
    const pending = [
    ...
    while (pending.length > 0) {
        ...
        if (directory.relative.length === 0)
            await verifyTrustedRoot(options.config, fs);
        else
            await vaultPaths.resolveDirectory(directory.relative, { automatic: true });
        ...
        for (const entry of entries) {
            visitedEntries += 1;
            if (visitedEntries > 4_096)
                throw new Error("entry scan budget exceeded");
            ...
            if ((!entry.isFile() && !entry.isSymbolicLink()) || !entry.name.endsWith(".md"))
                continue;
            if (/\.sync-conflict-/iu.test(entry.name))
                continue;
            let resolved;
            try {
            ...
            if (files.length > 4_096)
                throw new Error("note scan budget exceeded");
        }
    }
...
function cachedNoteIsValid(note) {
    try {
...
async function loadIndex(options, fs, vaultPaths, cache) {
    ...
    if (cached?.vault.dev !== identity.dev || cached.vault.ino !== identity.ino)
        cached = null;
...
function relevantExcerpt(note, query) {
    let searchOffset = 0;
    for (const line of note.source.split(/\r\n|\r|\n|\u2028|\u2029/u)) {
        ...
        while (searchOffset < note.source.length &&
...
export async function searchVault(options) {
    if (typeof options.query !== "string")
        throw new SearchError("invalid_query");
    ...
    const requested = Number.isSafeInteger(options.limit) ? options.limit : MAX_SEARCH_HITS;
...
export async function readVaultNote(options) {
    ...
    try {
        ...
        if (!before.isFile() || !fileStat.isFile() ||
    ...
    if (options.heading !== undefined) {
        if (typeof options.heading !== "string")
            throw new VaultReadError("invalid_heading");
        const extraction = extractSection(source, options.heading);
        if (extraction.kind === "invalid_heading")
            throw new VaultReadError("invalid_heading");
        if (extraction.kind === "missing")
            throw new VaultReadError("heading_missing");
