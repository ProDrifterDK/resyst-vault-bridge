 ...
 */
export function safeReadStringProperty(source, key, maxLength) {
...
export function authorityFromHeader(header) {
...
export class BootstrapLoopCache {
    ...
    load(session, prompt, loader) {
        ...
        const settled = leader.then(() => undefined, () => undefined);
        this.entries.set(key, { status: "pending", token, promise: settled });
        return leader.then((value) => {
    ...
    makeRoom() {
        if (this.entries.size < MAX_CACHE_KEYS)
            return true;
