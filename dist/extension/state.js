...
import { CHECKPOINT_OUTCOMES } from "../checkpoint-contract.js";
/** Custom-entry type used for transcript mirrors. */
export const CHECKPOINT_STATE_CUSTOM_TYPE = "resyst-vault.checkpoint-state";
...
const CHECKPOINT_STATE_ERROR_MESSAGE = "checkpoint state is invalid; details redacted";
/**
...
export class CheckpointStateError extends Error {
    code;
    constructor(code = "invalid_state") {
...
function invalidState(code = "invalid_state") {
    return new CheckpointStateError(code);
}
function safeArrayKind(value) {
    try {
        return Array.isArray(value) ? "array" : "not_array";
    }
    catch {
        return "invalid";
    }
...
function exactOwnKeys(value, allowed) {
    try {
        const names = Object.getOwnPropertyNames(value);
        const symbols = Object.getOwnPropertySymbols(value);
        if (symbols.length !== 0 || names.length !== allowed.length)
            return false;
...
function isCheckpointOutcome(value) {
    return typeof value === "string" && CHECKPOINT_OUTCOMES.includes(value);
}
function canonicalRecord(sessionId, revision, state, updatedAt) {
    if (!boundedSessionId(sessionId) || !Number.isSafeInteger(revision) || revision < 0 || revision > MAX_CHECKPOINT_REVISION) {
        throw invalidState();
    }
    if (!boundedTimestamp(updatedAt))
        throw invalidState();
 ...
 */
export function parseCheckpointStateRecord(value) {
    ...
    if (substantial.value !== flags.substantial ||
...
export function isCheckpointStateRecord(value) {
    try {
...
export function initialCheckpointRecord(sessionId, now) {
    if (!boundedSessionId(sessionId) || !boundedTimestamp(now))
        throw invalidState();
    return canonicalRecord(sessionId, 0, "clean", now);
}
function parseStateEvent(value) {
    if (!isPlainRecord(value))
        throw invalidState();
    const kind = ownDataProperty(value, "kind");
    if (!kind.present || typeof kind.value !== "string" || !EVENT_KINDS.includes(kind.value)) {
        throw invalidState();
    }
    if (kind.value === "checkpoint_outcome") {
        if (!exactOwnKeys(value, ["kind", "outcome", "basis_revision"]))
            throw invalidState();
        const outcome = ownDataProperty(value, "outcome");
        const basis = ownDataProperty(value, "basis_revision");
        if (!outcome.present || !isCheckpointOutcome(outcome.value) ||
    ...
    if (!exactOwnKeys(value, ["kind"]))
        throw invalidState();
    if (kind.value === "substantial")
        return { kind: "substantial" };
    if (kind.value === "uncertain")
        return { kind: "uncertain" };
    if (kind.value === "begin_evaluation")
        return { kind: "begin_evaluation" };
    if (kind.value === "evaluation_incomplete")
        return { kind: "evaluation_incomplete" };
    return { kind: "evaluation_completed" };
}
function stateAfterEvent(current, currentRevision, event) {
    switch (event.kind) {
        case "substantial":
        ...
        case "checkpoint_outcome":
            if (event.basis_revision !== currentRevision &&
 ...
 */
export function reduceCheckpointState(record, event, now) {
    const current = parseCheckpointStateRecord(record);
    const parsedEvent = parseStateEvent(event);
    if (!boundedTimestamp(now))
        throw invalidState();
    if (current.revision >= MAX_CHECKPOINT_REVISION)
        throw invalidState();
    return canonicalRecord(current.session_id, current.revision + 1, stateAfterEvent(current.state, current.revision, parsedEvent), now);
}
...
function observationProperty(value, key) {
    try {
...
function firstCommandWord(segment) {
    ...
    if (words === null || words[0] === undefined)
        return null;
...
function classifyBashSegment(segment) {
    ...
    if (first === "git") {
        ...
        if (MUTATING_GIT_SUBCOMMANDS.has(gitSubcommand)) {
            if (gitSubcommand === "tag" && /(?:^|\s)(?:-l|--list)(?:\s|$)/u.test(remainder))
                return "ignore";
            return "substantial";
        }
        return "uncertain";
    ...
    if (first === "sed")
        return /(?:^|\s)(?:-i(?:[^\s]*)?|--in-place(?:=[^\s]*)?)(?:\s|$)/u.test(trimmed) ? "substantial" : "ignore";
    if (first === "find") {
        if (/(?:^|\s)-delete(?:\s|$)/u.test(trimmed))
            return "substantial";
        if (/(?:^|\s)-(?:exec|execdir|ok|okdir)(?:\s|$)/u.test(trimmed))
            return "uncertain";
        return "ignore";
    }
    if (MUTATING_EXECUTABLES.has(first))
        return "substantial";
    if (READ_ONLY_EXECUTABLES.has(first))
        return "ignore";
    if (first === "deploy" || first === "release" || first === "terraform" || first === "kubectl") {
        const lower = trimmed.toLowerCase();
        if (/\b(?:plan|validate|get|describe|version|diff)\b/u.test(lower))
            return "ignore";
        return "substantial";
    }
...
export class EffectTracker {
    ...
    observe(observation) {
        ...
        if (!boundedToolName(toolName) || typeof isError !== "boolean")
            return "uncertain";
        if (isError)
            return "ignore";
        if (BRIDGE_TOOL_NAMES.has(toolName) || HOST_READ_TOOL_NAMES.has(toolName))
            return "ignore";
        if (this.substantialTools.has(toolName) || toolName === "edit")
            return "substantial";
        if (toolName === "bash")
            return classifyBash(commandProperty(input));
        return "uncertain";
    }
...
async function ensureStateBase(stateRoot) {
...
async function ensurePendingDirectory(stateRoot) {
...
async function readBoundedUtf8(handle) {
...
export class PendingStateStore {
    ...
    async current(sessionId) {
        ...
        if (volatile !== undefined)
            return cloneStateRecord(volatile);
        return await this.load(sessionId);
    }
    hasVolatilePending(sessionId) {
    ...
    async update(sessionId, reducer) {
        if (!boundedSessionId(sessionId) || typeof reducer !== "function")
            throw invalidState();
    ...
    async save(record) {
        ...
        if (!/^[A-Za-z0-9_-]{1,128}$/u.test(suffix))
            throw invalidState();
        const temporary = path.join(directory.path, `.${path.basename(destination)}.tmp-${process.pid}-${suffix}`);
        let handle;
...
function cloneStateRecord(record) {
    return {
...
function mirrorIsPending(state) {
 ...
 */
export function reconcileCheckpointState(input) {
