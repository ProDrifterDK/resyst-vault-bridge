...
export const SCHEMA_REJECTED_MESSAGE = "rejected by versioned schema; details redacted";
/** Fixed body of the evidence-citation cross-validation message. */
export const EVIDENCE_CITATION_ERROR_MESSAGE = "evidence citations must reference known evidence ids";
/** Fixed body of the duplicate-evidence message. */
export const DUPLICATE_EVIDENCE_ERROR_MESSAGE = "evidence ids must be unique across all evidence collections";
/** Fixed error for a payload rejected by a versioned schema. */
export class SchemaValidationError extends Error {
    subject;
    code;
    constructor(subject, message = `invalid ${subject}: ${SCHEMA_REJECTED_MESSAGE}`, code = "SCHEMA_VALIDATION_ERROR") {
        super(message);
 ...
 * validation.
 */
export function parseWithSchema(schema, value, subject) {
    try {
...
export const SessionIdSchema = Codec(Type.String({ minLength: 1, maxLength: 128, pattern: ID_PATTERN }))
    .Decode((value) => value)
    .Encode((value) => value);
export const HostIdSchema = Codec(Type.String({ minLength: 1, maxLength: 128, pattern: ID_PATTERN }))
    .Decode((value) => value)
    .Encode((value) => value);
export const ProjectIdSchema = Codec(Type.String({ minLength: 1, maxLength: 128, pattern: ID_PATTERN }))
    .Decode((value) => value)
    .Encode((value) => value);
export const EventIdSchema = Codec(Type.String({ minLength: 1, maxLength: 128, pattern: ID_PATTERN }))
    .Decode((value) => value)
    .Encode((value) => value);
export const EvidenceIdSchema = Codec(Type.String({ minLength: 1, maxLength: 128, pattern: ID_PATTERN }))
    .Decode((value) => value)
    .Encode((value) => value);
 ...
 */
export const IdempotencyKeySchema = Codec(Type.String({
    minLength: 64,
    maxLength: 64,
    ...
    .Decode((value) => value)
    .Encode((value) => value);
export const VaultPathSchema = Codec(Type.String({ minLength: 1, maxLength: 1024, pattern: VAULT_PATH_PATTERN }))
    .Decode((value) => value)
    .Encode((value) => value);
export const HashHexSchema = Codec(Type.String({ pattern: SHA256_HEX_PATTERN }))
    .Decode((value) => value)
    .Encode((value) => value);
export const IsoTimestampSchema = Codec(Type.String({ format: "date-time", maxLength: 64 }))
    .Decode((value) => value)
    .Encode((value) => value);
...
// they drift apart.
export const _noopReasonAligned = true;
export const _resolutionBasisAligned = true;
export const _unresolvedReasonAligned = true;
export const _deferReasonAligned = true;
export const _failReasonAligned = true;
export const _searchMatchFieldAligned = true;
...
export const ProjectRefSchema = Type.Object({ id: ProjectIdSchema }, { additionalProperties: false });
...
export const _receiptOutcomeAligned = true;
 ...
 */
function assertUniqueEvidenceIds(checkpoint) {
    const seen = new Set();
    const collections = [
    ...
    for (const collection of collections) {
        for (const item of collection) {
            if (seen.has(item.id)) {
                throw new SchemaValidationError("checkpoint", `invalid checkpoint: ${DUPLICATE_EVIDENCE_ERROR_MESSAGE}`, "DUPLICATE_EVIDENCE_ERROR");
            }
            seen.add(item.id);
        }
    }
...
function assertKnownEvidenceCitations(checkpoint) {
    const known = new Set();
    ...
    for (const collection of knowledgeCollections) {
        for (const item of collection) {
            for (const cited of item.evidence) {
                if (!known.has(cited)) {
                    throw new SchemaValidationError("checkpoint", `invalid checkpoint: ${EVIDENCE_CITATION_ERROR_MESSAGE}`, "EVIDENCE_CITATION_ERROR");
                }
            }
        }
    }
 ...
 * can be persisted.
 */
export function parseJournalEvent(value) {
...
export function parseProjectResolution(value) {
...
export function parseBootstrapResult(value) {
...
export function parseSearchHit(value) {
