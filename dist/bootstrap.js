...
import { IsoTimestampSchema, ProjectResolutionSchema, VaultPathSchema, parseBootstrapResult, parseWithSchema, } from "./schemas.js";
import { extractSection, parseNote } from "./markdown.js";
export const BOOTSTRAP_SAFETY_PREFIX = "RESYST VAULT BRIDGE — ROOT-TURN CONTEXT\n";
...
function makeEstimator(estimator) {
    const state = {
        healthy: true,
        estimate(value) {
            if (!state.healthy)
...
function narrowSnapshot(snapshot) {
    if (snapshot === null ||
    ...
    try {
        return {
            ...
            modified_at: parseWithSchema(IsoTimestampSchema, snapshot.modified_at, "bootstrap snapshot modification time"),
        };
    }
    catch {
        return null;
...
function noteIsStructurallyValid(note) {
    return note.frontmatter.kind !== "invalid" && note.managed.kind === "ok";
}
function sectionWithoutNestedHistory(note, extraction) {
    const selected = note.headings.find((heading) => heading.start === extraction.heading_start);
    if (!selected)
        return note.source.slice(extraction.heading_start, extraction.section_end);
    const exclusions = [];
...
function lineEndAfter(value, offset) {
    let cursor = offset;
    while (cursor < value.length &&
        value[cursor] !== "\n" &&
        value[cursor] !== "\r" &&
...
function safeBoundaries(value) {
    const lineBoundaries = new Set([0, value.length]);
    for (const line of lineRanges(value))
        lineBoundaries.add(line.end);
    const boundaries = new Set(lineBoundaries);
    ...
    for (const line of lineRanges(value)) {
        if (MARKER_LIKE.test(line.text))
            blocked.push({ start: line.start, end: line.end });
    }
    ...
    const sortedBlocked = blocked.sort((left, right) => left.start === right.start ? left.end - right.end : left.start - right.start);
    const safe = [];
    let blockedIndex = 0;
    for (const boundary of sortedBoundaries) {
        if (boundary > 0 &&
            ...
            value[boundary] === "\n")
            continue;
        while (blockedIndex < sortedBlocked.length &&
            (sortedBlocked[blockedIndex]?.end ?? 0) <= boundary) {
            blockedIndex += 1;
        }
        const range = sortedBlocked[blockedIndex];
        if (range !== undefined && boundary > range.start && boundary < range.end)
            continue;
        safe.push(boundary);
    }
    return safe;
}
function renderFragment(snapshot, heading, content) {
    return [
...
function claudeCandidates(snapshot) {
    const note = parseNote(snapshot.source);
    if (!noteIsStructurallyValid(note))
...
function projectCandidates(snapshot) {
    ...
    for (const requested of PROJECT_SECTIONS) {
        ...
        if (selected === undefined || selected.content.trim().length === 0)
            continue;
        candidates.push({
...
function dailyCandidates(snapshot, headings) {
    const note = parseNote(snapshot.source);
    if (!noteIsStructurallyValid(note))
        return { candidates: [], truncated: true };
...
function collectCandidates(input, project) {
    ...
    const projectSelected = project.kind === "resolved" &&
...
function fragmentRecord(candidate, content, truncated) {
    return {
...
function chooseTruncatedContent(candidate, current, minimumEstimate, budget, estimator) {
    const boundaries = safeBoundaries(candidate.content);
    const candidates = boundaries.filter((boundary) => boundary > 0 && boundary < candidate.content.length);
    let low = 0;
    ...
    while (low <= high) {
        ...
        const fits = current.length + rendered.length <= MAX_CONTEXT_CHARS && estimate <= budget;
        if (fits) {
...
export function buildBootstrap(input) {
    const budget = budgetFrom(input.config);
    const project = parseWithSchema(ProjectResolutionSchema, input.project, "bootstrap project resolution");
    ...
    for (const candidate of selected.candidates) {
        const fullRendered = renderFragment(candidate.snapshot, candidate.heading, candidate.content);
        ...
        const shortened = chooseTruncatedContent(candidate, context, estimatedTokens, budget, estimator);
        if (shortened === null) {
            truncated = true;
            continue;
        }
