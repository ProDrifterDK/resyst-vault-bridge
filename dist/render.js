...
import { findHeading, insertManagedBlock, replaceManagedBlock, } from "./markdown.js";
...
function evidenceMap(checkpoint) {
    const map = new Map();
    for (const collection of Object.values(checkpoint.evidence)) {
        for (const item of collection)
            map.set(item.id, safeText(item.value));
    }
    return map;
}
function linesFor(items, evidence, prefix) {
    const lines = [];
    for (const item of items) {
        ...
        for (const id of item.evidence) {
            ...
            if (value !== undefined)
                lines.push(`  - evidencia: ${value}`);
        }
    }
    return lines;
}
function dailyBodies(input) {
    ...
    return {
        ...
        enlaces: input.resolution.kind === "resolved" &&
...
function upsert(source, heading, sessionId, target, body) {
    const replaced = replaceManagedBlock(source, sessionId, target, body);
    if (replaced.kind === "replaced")
        return { ok: true, source: replaced.source };
    if (replaced.kind !== "not_found")
        return { ok: false };
...
function plan(path, before, after, reason) {
    return {
...
export function buildWritePlans(input) {
    ...
    if (knowledgeCharacters + evidenceCharacters > MAX_RENDER_SOURCE_CHARS ||
        (input.project_title !== null &&
    ...
    if (input.effective_targets.daily) {
        ...
        if (before === null) {
            const templateHeadings = DAILY_FRAGMENTS.map((fragment) => findHeading(next, input.config.managed_headings[fragment.key]));
            if (templateHeadings.some((heading) => heading.kind !== "found")) {
                const conflictPath = parseWithSchema(VaultPathSchema, `${input.config.daily_dir}/${input.date}.md`, "daily path");
        ...
        for (const fragment of DAILY_FRAGMENTS) {
            ...
            const updated = upsert(next, input.config.managed_headings[fragment.key], input.checkpoint.source.session_id, fragment.target, body);
            if (!updated.ok) {
                const conflictPath = parseWithSchema(VaultPathSchema, `${input.config.daily_dir}/${input.date}.md`, "daily path");
                return { kind: "deferred", reason: "conflict", conflict_paths: [conflictPath] };
            }
        ...
        if (next !== before) {
            plans.push(plan(`${input.config.daily_dir}/${input.date}.md`, before, next, before === null ? "daily_create" : "daily_update"));
        }
    }
    if (input.effective_targets.project) {
        ...
        if (body.length > 0) {
            ...
            const projectHeadings = ["## Estado actual", "## Estado"];
            const foundHeadings = projectHeadings.filter((heading) => findHeading(source, heading).kind === "found");
    ...
    if (input.effective_targets.landscape) {
        ...
        if (latestStatus !== undefined)
            parts.push(`estado: ${safeText(latestStatus)}`);
        if (latestNext !== undefined)
            parts.push(`siguiente: ${safeText(latestNext)}`);
        ...
        for (const target of landscapeTargets) {
            ...
            const updated = upsert(target.source, target.heading, input.checkpoint.source.session_id, "landscape", body);
            if (!updated.ok) {
                return {
                    kind: "deferred",
                    reason: "landscape_ambiguous",
