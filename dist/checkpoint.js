...
const EVIDENCE_KEYS = ["commits", "tests", "files", "deployments", "observations"];
function inputIsBounded(value) {
...
function canonicalApply(checkpoint) {
    const knowledge = {
        completed_tasks: [],
        decisions: [],
        status_changes: [],
    ...
    for (const key of KNOWLEDGE_KEYS) {
        ...
        for (const item of checkpoint.knowledge[key]) {
            ...
            if (normalized.text.length === 0 || seen.has(normalized.text))
                continue;
...
function meaningful(checkpoint) {
    return KNOWLEDGE_KEYS.some((key) => checkpoint.knowledge[key].length > 0);
}
function evidencePolicy(checkpoint) {
    for (const item of [
    ...
    const cited = new Set(KNOWLEDGE_KEYS.flatMap((key) => checkpoint.knowledge[key].flatMap((item) => item.evidence)));
...
function idempotencyKey(checkpoint) {
...
export function normalizeCheckpoint(value, project) {
    let parsed;
    try {
        if (!inputIsBounded(value))
    ...
    if (!meaningful(checkpoint))
        return { kind: "invalid", reason: "empty_apply" };
    ...
    if (evidenceFailure !== null)
        return { kind: "invalid", reason: evidenceFailure };
    const effectiveTargets = { ...checkpoint.targets };
    let downgrade = null;
    const projectMatches = project.kind === "resolved" &&
    ...
    const landscapeChanged = checkpoint.knowledge.status_changes.length > 0 ||
