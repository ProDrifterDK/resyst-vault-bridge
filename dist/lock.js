...
async function ensureParentChain(root, directory) {
...
export class LocalLock {
    ...
    async ownerForAcquire() {
        ...
        if (!ISO_RE.test(now) || !Number.isFinite(new Date(now).getTime()))
            throw new LockIntegrityError();
        return { version: 1, pid: this.pid, process_start: start, acquired_at: now };
    }
    async ownerLive(owner) {
    ...
    async readOwner() {
        ...
        try {
            ...
            if (Buffer.byteLength(raw, "utf8") > MAX_OWNER_BYTES)
                throw new LockIntegrityError();
            let parsed;
            try {
            ...
            if (errorCode(error) === "ENOENT")
                return null;
            if (error instanceof LockIntegrityError)
                throw error;
            throw new LockIntegrityError();
        }
        finally {
    ...
    async tryRemoveAbandoned(owner) {
        let handle;
        try {
            handle = await open(this.path, "r", O_NOFOLLOW);
            const ownerStat = await handle.stat({ bigint: true });
            ...
            release: async () => {
                ...
                try {
                    ...
                    if (failure === undefined)
                        failure = new LockIntegrityError();
                }
                if (failure instanceof LockIntegrityError)
                    throw failure;
                if (failure !== undefined)
                    throw new LockIntegrityError();
            },
    ...
    async removeAbandoned() {
        ...
        if (live === true)
            return "not_abandoned";
        if (live === "unknown")
            return "refused_unknown_liveness";
        try {
    ...
    async acquire() {
        ...
        while (true) {
            ...
            if (acquired !== null && acquired !== "temp_collision")
                return acquired;
            ...
            if (existing !== null) {
                ...
                if (live === false)
                    await this.tryRemoveAbandoned(existing);
            }
            const afterReadMs = this.nowMs();
            if (!Number.isFinite(afterReadMs))
                throw new LockIntegrityError();
            if (afterReadMs >= deadline)
                throw new LockTimeoutError();
            try {
