...
export const MARKER_TEXT_REJECTED_MESSAGE = "managed body contains marker-like text; rejected";
/** Split source into lines with exact offsets; handles LF and CRLF. */
function lineInfos(source) {
    const lines = [];
    let i = 0;
...
function narrowRequiredStringArray(value) {
    if (value === undefined) {
        return [];
    }
 ...
 */
function narrowStringList(value) {
...
const BEGIN_MARKER = /^<!--\s*resyst-vault:begin\s+session=([A-Za-z0-9][A-Za-z0-9._-]{0,127})\s+target=([A-Za-z0-9][A-Za-z0-9._-]{0,127})\s*-->$/;
const END_MARKER = /^<!--\s*resyst-vault:end\s+session=([A-Za-z0-9][A-Za-z0-9._-]{0,127})\s+target=([A-Za-z0-9][A-Za-z0-9._-]{0,127})\s*-->$/;
/** Build the closing-fence matcher for an open fence char and length. */
function closingFence(char, length) {
 ...
 */
function scanDocument(source, lines, frontmatter) {
    const headings = [];
    const fences = [];
    const indented = [];
    const completed = [];
 ...
 */
function parseWikilink(inner, start, end) {
    if (inner === "" ||
        inner.includes("[[") ||
        inner.includes("<!--") ||
        inner.includes("-->")) {
 ...
 */
function inlineCodeRegions(text) {
    const regions = [];
 ...
 */
function scanLineLinks(text, base, codeRegions) {
    const links = [];
    const len = text.length;
    let regionIdx = 0;
    let noCloserAfter = -1;
...
function sortAndCoalesce(regions) {
 ...
 */
function scanWikilinks(lines, excluded) {
    const links = [];
    let regionIdx = 0;
    for (const line of lines) {
...
function matchingHeadings(note, spec) {
    return note.headings.filter((heading) => heading.level === spec.level && heading.text === spec.text);
}
/** Look up a heading by full heading text (`## Tareas`). */
export function findHeading(source, heading) {
...
function containsMarkerLikeText(text) {
 ...
 */
export function replaceManagedBlock(source, sessionId, target, body) {
    ...
    const matches = note.managed.blocks.filter((block) => block.session_id === sessionId && block.target === target);
    if (matches.length === 0) {
    ...
    const next = source.slice(0, block.body_start) +
        normalized +
 ...
 */
export function insertManagedBlock(source, heading, sessionId, target, body) {
    if (!isSafeId(sessionId) || !isSafeId(target)) {
        return { kind: "invalid_id" };
    }
    if (containsMarkerLikeText(body)) {
    ...
    const exists = note.managed.blocks.some((block) => block.session_id === sessionId && block.target === target);
    if (exists) {
    ...
    const next = source.slice(0, selected.line_end) +
        eol +
        blockText +
    ...
    const block = {
        ...
        body_end: beginStart + beginMarker.length + eol.length + normalized.length,
